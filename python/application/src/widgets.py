...
def createItemButtonIcon(item: MarketItem, callback: Callable[[MarketItem], None]) -> QPushButton:
    ...
    buttonIcon = qt_resource.createButton(buttonIconName, "", qt_resource.fontSystemHudBold, buttonIconSheet)
...
def createItemNameLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelName = qt_resource.createLabel(f"itemLabelName{item.tempID}", f"{item.weaponName} {item.skinName}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelName

def createItemNameCategoryWearLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelWearCategory = qt_resource.createLabel(f"itemLabelCategoryWear{item.tempID}", f"{definitions.wearToString(item.wear)} | {definitions.categoryToString(item.category)}", qt_resource.fontSystemHudBold, alignment)
...
def createItemNameCategoryWearGradeLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelWearCategory = qt_resource.createLabel(f"itemLabelCategoryWearGrade{item.tempID}", f"{definitions.wearToString(item.wear)} | {definitions.categoryToString(item.category)} | {definitions.gradeToString(item.grade)}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelWearCategory

def createItemMarketPriceLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelPrice = qt_resource.createLabel(f"itemLabelPrice{item.tempID}", f"Market Price ${str(item.marketPrice)}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelPrice  

def createModifiedPriceLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelPrice = qt_resource.createLabel(f"modifiedLabelPrice{item.tempID}", f"Modified Price ${str(item.modifiedPrice)}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelPrice  

def createItemCurrentPriceLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelPrice = qt_resource.createLabel(f"currentLabelPrice{item.tempID}", f"Current Price ${str(item.price)}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelPrice  

def createItemSteamTaxPriceLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemLabelPrice = qt_resource.createLabel(f"priceLabelSteamTaxPrice{item.tempID}", f"Current Price Steam Tax ${str(item.priceSteamTax)}", qt_resource.fontSystemHudBold, alignment)
    return itemLabelPrice  

def createItemSteamMarketURLLabel(item: MarketItem, alignment: Optional[Qt.AlignmentFlag] = None) -> QLabel:
    itemSteamMarketURL = qt_resource.createLabel(f"steamMarketURL{item.tempID}", f"<a style='color:red;' href=\"{item.steamMarketUrl}\">Steam Market URL</a>", qt_resource.fontSystemHudBold, alignment)
...
def createTradeupDateTimeLabel(tradeup: Tradeup, alignment: Optional[Qt.AlignmentFlag] = None):
    dateTimeLabel = qt_resource.createLabel("dateTimeLabel", f"{tradeup.dateFound}", qt_resource.fontSystemHudBold, alignment)
    return dateTimeLabel

def createProfitabilityLabel(tradeup: Tradeup, alignment: Optional[Qt.AlignmentFlag] = None):
    profLabel = qt_resource.createLabel("profitabilityLabel", f"Profitability {int(tradeup.profitability)}% - {int(tradeup.profitabilitySteamTax)}% TAX", qt_resource.fontSystemHudBold, alignment)
    return profLabel

def createChanceToProfitLabel(tradeup: Tradeup, alignment: Optional[Qt.AlignmentFlag] = None):
    chanceToProfLabel = qt_resource.createLabel("chanceToProfitLabel", f"Profit Chance {int(tradeup.chanceToProfit)}% - {int(tradeup.chanceToProfitSteamTax)}% TAX", qt_resource.fontSystemHudBold, alignment)
    return chanceToProfLabel

def createFloatValLabel(floatVal: float, alignment: Optional[Qt.AlignmentFlag] = None):
    chanceToProfLabel = qt_resource.createLabel("floatValLabel", f"Float Value {round(floatVal, 2)}", qt_resource.fontSystemHudBold, alignment)
    return chanceToProfLabel

def createEntryItemTradeupChanceLabel(tradeupChance: float, alignment: Optional[Qt.AlignmentFlag] = None):
    tradeupChanceLabel = qt_resource.createLabel("tradeupChanceLabel", f"Tradeup Chance {round(tradeupChance, 2)}%", qt_resource.fontSystemHudBold, alignment)
...
def createEntryItemMoneyGainLabel(moneyGain: float, moneyGainSteamTax: float, alignment: Optional[Qt.AlignmentFlag] = None):
    moneyGainLabel = qt_resource.createLabel("moneyGainLabel", f"Gain ${round(moneyGain, 2)} - ${round(moneyGainSteamTax, 2)} TAX", qt_resource.fontSystemHudBold, alignment)
    return moneyGainLabel

def createInputPriceLabel(tradeup: Tradeup, alignment: Optional[Qt.AlignmentFlag] = None):
    inputPriceLabel = qt_resource.createLabel("inputPriceLabel", f"Input Price ${round(tradeup.totalInputCost, 3)}", qt_resource.fontSystemHudBold, alignment)
